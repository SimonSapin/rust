...
impl<'tcx> Ty<'tcx> {
    ...
    /// Returns the type of metadata for (potentially fat) pointers with a tail of this
    /// type and a boolean signifying if this is conditional on this type being `Sized`.
    pub fn metadata_type_as_tail(self, tcx: TyCtxt<'tcx>) -> (Ty<'tcx>, bool) {
        match self.kind() {
            // Sized types
